...
#ifdef USE_ROCM
// Work around ROCm 7.0 accuracy regressions for fp32 output with f16/bf16 inputs
// on small-N batched GEMMs.
static bool rocm_use_fp32_baddbmm_fallback(
    const Tensor& batch1,
    const Tensor& batch2,
    const Tensor& result) {
  ...
  const auto dtype = batch1.scalar_type();
  if (dtype != at::kHalf && dtype != at::kBFloat16) {
    return false;
  }
  if (batch2.scalar_type() != dtype || batch1.dim() != 3 || batch2.dim() != 3) {
    return false;
  }
  const auto b = batch1.size(0);
  const auto m = batch1.size(1);
  const auto k = batch1.size(2);
  const auto n = batch2.size(2);

  if (dtype == at::kBFloat16) {
    return n == 1 && (k == 32 || k == 64);
  }
  return b == 1 && n == 1 && k == 64 && (m == 32 || m == 64);
}
#endif
...
#ifdef USE_ROCM
  if (rocm_use_fp32_baddbmm_fallback(batch1, batch2, result)) {
    const auto beta_val = beta.to<c10::complex<double>>();
    Tensor self_fp32 = (beta_val == c10::complex<double>(0.0, 0.0))
        ? at::zeros_like(result, result.options().dtype(at::kFloat))
        : (self.scalar_type() == at::kFloat ? self : self.to(at::kFloat));
    Tensor batch1_fp32 = batch1.to(at::kFloat);
    Tensor batch2_fp32 = batch2.to(at::kFloat);
    Tensor out_fp32 = at::baddbmm(self_fp32, batch1_fp32, batch2_fp32, beta, alpha);
    result.copy_(out_fp32);
    return result;
  }
#endif
